...
function resolveConfigFilePath (project_dir, platform, file) {
    ...
    if (platform === 'android' && !fs.existsSync(filepath)) {
        if (file === 'AndroidManifest.xml') {
            filepath = path.join(project_dir, 'deviceReady', 'src', 'main', 'AndroidManifest.xml');
        } else if (file.endsWith('config.xml')) {
            filepath = path.join(project_dir, 'deviceReady', 'src', 'main', 'res', 'xml', 'config.xml');
        } else if (file.endsWith('strings.xml')) {
            // Plugins really shouldn't mess with strings.xml, since it's able to be localized
            filepath = path.join(project_dir, 'deviceReady', 'src', 'main', 'res', 'values', 'strings.xml');
            ...
            filepath = path.join(project_dir, 'deviceReady', 'src', 'main', 'res', 'xml', config_file);
        }
        return filepath;
    }
