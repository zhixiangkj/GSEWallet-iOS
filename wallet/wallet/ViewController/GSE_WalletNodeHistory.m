...
@interface GSE_WalletNodeHistory ()<IGListAdapterDataSource, UIScrollViewDelegate, UICollectionViewDelegate>
...
@implementation GSE_WalletNodeHistory

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.view.backgroundColor = [UIColor whiteColor];
    self.title = NSLocalizedString(@"权益委托详情",nil);
    ...
    [self reloadData:nil finish:^{
        if (weakSelf.items.count <= 1) {
            weakSelf.emptyLabel.text = NSLocalizedString(@"没有委托，没有收益",nil);
...
- (void)reloadData{
    ...
    if ([array isKindOfClass:[NSArray class]] && array.count) {
        ...
        [array enumerateObjectsUsingBlock:^(NSDictionary *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            GSE_NodeStake * item = [[GSE_NodeStake alloc] init];
...
- (void)reloadData:(NSString *)lastid finish:( void (^)(void) ) finish{
    ...
    __weak typeof(self)weakSelf = self;
    [[provider getStakeHistory:dic] onCompletion:^(DictionaryPromise *promise) {
        if (!promise.result) {
...
- (IGListSectionController *)listAdapter:(IGListAdapter *)listAdapter sectionControllerForObject:(id)object {
    ...
    GSE_NodeStakeController *controller = [[GSE_NodeStakeController alloc] init];
...
- (void)addMoreData{
    ...
    if (self.items.count > 1) {
        GSE_NodeStake *info = self.items.lastObject;
        [self reloadData:info.stakeid finish:nil];
    }else{
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    GSE_NodeStake *item = self.items[indexPath.section];
    
    if (![item isKindOfClass:[GSE_NodeStake class]]) {
        return;
    }
